          ...
          <div className="mb-8 flex gap-2 md:gap-4 border-b border-outline-variant overflow-x-auto">
            {tabs.map((tab) => (
              <button
                ...
                className={`px-3 md:px-4 py-3 font-headline font-bold border-b-2 transition-colors text-sm md:text-base whitespace-nowrap ${activeTab === tab
            ...
            <div className="bg-white rounded-xl border border-outline-variant p-4 md:p-8 animate-fade-in">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <p className="text-on-surface-variant text-sm mb-4">Manage your recent login sessions</p>
                  <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                    {sessions.length === 0 ? (
                       <div className="p-3 text-sm text-on-surface-variant text-center bg-surface-container rounded">No recent sessions found.</div>
                    ) : (
                      sessions.map((sess, idx) => (
                        <div key={sess.id} className="flex justify-between items-center p-3 bg-surface-container rounded mb-2 border border-outline-variant/30">
                          <div>
                            <span className="block text-on-surface font-bold text-sm md:text-base">{sess.device}</span>
                            <span className="block text-xs text-on-surface-variant">{new Date(sess.timestamp).toLocaleString()}</span>
                          </div>
                          {idx === 0 ? <span className="text-sm font-bold text-green-600 bg-green-50 px-2 py-1 rounded">Active Now</span> : <span className="text-sm text-on-surface-variant bg-slate-50 px-2 py-1 rounded">Past Session</span>}
                        </div>
                      ))
                    )}
                  </div>
              ...
              <div className="space-y-6">
                <div className="p-4 md:p-6 bg-surface-container rounded-lg">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
                    <h3 className="font-headline font-bold text-on-surface">Current Plan: Free Plan</h3>
                    <span className="text-sm font-bold text-on-surface-variant">Never expires</span>
                  </div>
                  <p className="text-on-surface-variant mb-4">Enjoy basic features forever. Upgrade for premium tools.</p>
                  <button className="w-full sm:w-auto px-6 py-2 border-2 border-primary text-primary font-bold rounded-lg hover:bg-primary hover:text-white transition-colors">Upgrade Plan</button>
                </div>
                <div>
                  <h3 className="font-headline font-bold text-on-surface mb-4">Billing History</h3>
                  <div className="space-y-2">
                    <div className="p-3 text-sm text-on-surface-variant text-center bg-surface-container rounded">No billing history available on the Free Plan.</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-8 animate-fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-2xl border border-outline-variant p-6">
                  ...
                  <div className="relative">
                    <span className="material-symbols-outlined notranslate absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-lg" translate="no">search</span>
                    <input
                      type="text"
                      value={searchLang}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredLanguages.map((lang) => {
                  ...
                  return (
                    <div
                      key={lang.code}
                      className={`bg-white rounded-2xl border transition-all hover:shadow-md hover:border-primary/30 p-5 ${primaryLang === lang.code ? 'border-primary ring-2 ring-primary/10' : 'border-outline-variant'
                        }`}
                    >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-orange-50/50 border border-orange-100 rounded-3xl p-6">
                  <div className="flex items-center gap-2 mb-6">
                    <span className="material-symbols-outlined notranslate text-primary" translate="no">public</span>
