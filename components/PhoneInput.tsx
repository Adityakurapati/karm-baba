...
export default function PhoneInput({
  ...
  const handleSendOtp = async () => {
    ...
    try {
      ...
      const validateRes = await fetch('/api/validate-phone', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ number: value, countryCode }),
      });
      const validateData = await validateRes.json();
      if (!validateData.success || !validateData.valid) {
        setOtpError(validateData.error || 'This number is not valid. Please check and try again.');
      ...
      if (!sendData.success) {
        setOtpError(sendData.error || 'Failed to send OTP. Please try again.');
        setSendLoading(false);
        return;
      }

      // In dev mode, show OTP in console (remove in production)
      if (sendData.code) {
        console.log(`[DEV] OTP for ${fullPhone}:`, sendData.code);
      }
      ...
      <div className="flex gap-2">
        ...
        <div className="flex-1 relative">
          <input
            ...
            maxLength={10}
            disabled={disabled || isVerified}
            className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:border-primary bg-surface font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed transition-all ${
              isVerified
