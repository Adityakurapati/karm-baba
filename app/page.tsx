...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background dark:bg-slate-950 text-on-surface dark:text-slate-100 transition-colors duration-300 font-sans overflow-x-hidden">
      <style>{`
      ...
      {/* ══ HERO ══ */}
      {/* ══ HERO ══ */}
      <section className="relative pt-24 pb-20 px-6 md:px-12 overflow-hidden bg-orange-50 dark:bg-gradient-to-br dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
        {/* blobs - hidden in dark mode or adjusted */}
        <div className="absolute top-10 right-[6%] w-72 h-72 rounded-full pointer-events-none dark:opacity-20"
          style={{ background: '#f97316', opacity: 0.07, filter: 'blur(70px)' }} />
        <div className="absolute bottom-0 left-[4%] w-56 h-56 rounded-full pointer-events-none dark:opacity-20"
          style={{ background: '#fb923c', opacity: 0.06, filter: 'blur(55px)' }} />

        {/* floating stat cards */}
        <div className="hero-float absolute top-32 right-[3%] hidden xl:flex flex-col gap-1 p-4 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm rounded-2xl border border-orange-100 dark:border-orange-900/30 w-44 dark:shadow-lg shadow-xl"
          style={{ animationDelay: '0s' }}>
          <span className="text-[10px] text-on-surface-variant dark:text-slate-400 font-bold uppercase tracking-wide">Live Deals</span>
          <span className="text-3xl font-black text-primary">2,847</span>
          <span className="text-xs text-green-600 dark:text-green-400 font-bold">↑ +12% today</span>
        </div>
        <div className="hero-float-slow absolute bottom-20 right-[11%] hidden xl:flex flex-col gap-1 p-4 bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm rounded-2xl border border-orange-100 dark:border-orange-900/30 w-44 dark:shadow-lg shadow-xl"
          style={{ animationDelay: '1.5s' }}>
          <span className="text-[10px] text-on-surface-variant dark:text-slate-400 font-bold uppercase tracking-wide">Deal Volume</span>
          <span className="text-3xl font-black text-primary">$2B+</span>
          <span className="text-xs text-on-surface-variant dark:text-slate-400">Verified trades</span>
        </div>

        <div className="max-w-3xl mx-auto text-center relative z-10">
          {/* badge */}
          <div className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full mb-8 border border-orange-200 dark:border-orange-800 bg-orange-50 dark:bg-orange-950/80 text-primary">
            <span className="relative w-2 h-2 rounded-full bg-primary pulse-dot flex-shrink-0" />
            The Gold Standard for Global Trade
          </div>

          <h1 className="text-5xl md:text-6xl font-headline font-black tracking-tight text-slate-900 dark:text-white leading-[1.1] mb-6">
            Where Business Deals Don&apos;t
            <br />
            Just Start —{' '}
            <span className="text-primary" style={{ display: 'inline-block', minWidth: '13ch' }}>
              {displayed}<span className="animate-pulse text-slate-400 dark:text-slate-500">|</span>
            </span>
          </h1>

          <p className="text-lg text-slate-600 dark:text-slate-300 mb-10 max-w-xl mx-auto leading-relaxed">
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            <Link
              href="/register"
              className="shimmer-btn px-8 py-3.5 text-white font-headline font-bold rounded-xl hover:scale-105 active:scale-95 transition-transform shadow-lg shadow-orange-200 dark:shadow-orange-900/30"
            >
              Start Selling →
            </Link>
            <Link
              ...
              className="px-8 py-3.5 border-2 border-primary text-primary font-headline font-bold rounded-xl hover:bg-orange-50 dark:hover:bg-orange-950/40 hover:scale-105 active:scale-95 transition-all"
            >
              Post Requirement
            </Link>
          </div>
      ...
      {/* ══ TRUST BAR ══ */}
      <section className="border-y border-orange-100 dark:border-orange-900/30 bg-white dark:bg-slate-900 py-5 px-6 md:px-12">
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={item.label}
              className="flex items-center justify-center gap-2 text-on-surface-variant dark:text-slate-400 text-sm font-semibold py-2.5 px-3 rounded-xl hover:bg-orange-50 dark:hover:bg-orange-950/30 hover:text-primary transition-colors cursor-default">
        ...
        <div className="flex justify-center gap-3 mt-4 flex-wrap">
          {['GST Verified', 'Trusted Network', 'AI Screening'].map((badge) => (
            <span key={badge}
              className="px-3 py-1 bg-orange-50 dark:bg-orange-950/50 border border-orange-200 dark:border-orange-800 rounded-full text-xs font-bold text-primary dark:text-orange-400 hover:bg-primary hover:text-white hover:border-primary transition-all cursor-default flex items-center gap-1">
      ...
      {/* ══ HOW IT WORKS ══ */}
      <section className="py-20 px-6 md:px-12 bg-white dark:bg-slate-900">
        <div className="max-w-5xl mx-auto">
          <FadeIn className="text-center mb-14">
            <p className="text-primary text-xs font-bold uppercase tracking-widest mb-2">Process</p>
            <h2 className="text-3xl md:text-4xl font-headline font-black text-on-surface dark:text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0 relative">
            ...
            {[
              { step: '01', icon: 'verified', title: 'Get Verified', desc: 'Identity and business validation through our rigorous trust protocol to ensure high-intent participation.', bg: 'bg-blue-50 dark:bg-blue-950/50', border: 'border-blue-200 dark:border-blue-800', num: 'text-blue-600 dark:text-blue-400' },
              { step: '02', icon: 'hub', title: 'Get Matched', desc: 'Our execution engine matches your requirements with verified suppliers or buyers within minutes.', bg: 'bg-orange-50 dark:bg-orange-950/50', border: 'border-orange-200 dark:border-orange-800', num: 'text-primary' },
              { step: '03', icon: 'handshake', title: 'Close Deals', desc: 'Facilitate secure negotiations, documentation, and payments all within the KARM BABA workspace.', bg: 'bg-violet-50 dark:bg-violet-950/50', border: 'border-violet-200 dark:border-violet-800', num: 'text-violet-600 dark:text-violet-400' },
            ].map((item, i) => (
              <FadeIn key={item.step} delay={i * 120} className="flex flex-col items-center text-center p-8 group">
                <div className={`relative w-20 h-20 rounded-2xl flex items-center justify-center mb-5 border-2 ${item.bg} ${item.border} group-hover:scale-110 transition-transform duration-300`}>
                  <span className="material-symbols-outlined notranslate text-3xl dark:text-white" translate="no">{item.icon}</span>
                  <span className={`absolute -top-3 -right-3 w-6 h-6 rounded-full bg-white dark:bg-slate-800 border-2 ${item.border} text-xs font-black flex items-center justify-center ${item.num}`}>
                    {item.step}
                  </span>
                </div>
                <h3 className="text-lg font-headline font-bold text-on-surface dark:text-white mb-2">{item.title}</h3>
                <p className="text-on-surface-variant dark:text-slate-400 text-sm leading-relaxed">{item.desc}</p>
              </FadeIn>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-6 md:px-12 bg-slate-50 dark:bg-slate-900/50">
        <div className="max-w-5xl mx-auto">
          <FadeIn>
            ...
            <h2 className="text-3xl font-headline font-black text-on-surface dark:text-white mb-6">
          ...
          <div className="flex gap-2 flex-wrap mb-6">
            {industries.map((ind, i) => (
              <button key={ind} onClick={() => setActivePill(i)}
                className={`pill-tab px-4 py-1.5 rounded-full text-sm font-bold border ${activePill === i ? 'active shadow-md shadow-orange-100 dark:shadow-orange-900/30' : 'border-outline-variant dark:border-slate-700 text-on-surface-variant dark:text-slate-400 bg-white dark:bg-slate-800 hover:border-primary hover:text-primary'}`}>
                {ind}
              </button>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            {[
              ...
              <FadeIn key={item.title} delay={i * 100}>
                <div className="industry-card p-6 bg-white dark:bg-slate-800 rounded-2xl border border-outline-variant dark:border-slate-700 cursor-pointer h-full">
                  <div className="flex items-start justify-between mb-3">
                    <span className="material-symbols-outlined notranslate text-3xl dark:text-white" translate="no">{item.icon}</span>
                    <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-orange-50 dark:bg-orange-950/60 text-primary border border-orange-100 dark:border-orange-800">{item.tag}</span>
                  </div>
                  <h3 className="font-headline font-bold text-on-surface dark:text-white mb-1">{item.title}</h3>
                  <p className="text-on-surface-variant dark:text-slate-400 text-sm">{item.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
          ...
          <FadeIn delay={300}>
            <div className="industry-card p-6 bg-white dark:bg-slate-800 rounded-2xl border border-outline-variant dark:border-slate-700 flex items-center justify-between flex-wrap gap-4 cursor-default">
              <div className="flex items-center gap-4">
                <span className="material-symbols-outlined notranslate text-3xl dark:text-white" translate="no">ship</span>
                <div>
                  <h3 className="font-headline font-bold text-on-surface dark:text-white mb-0.5">Import-Export Services</h3>
                  <p className="text-on-surface-variant dark:text-slate-400 text-sm">Complete logistics, customs clearance, and trade financing for global commerce.</p>
                </div>
              </div>
              <Link href="/markets"
                className="px-5 py-2.5 bg-primary text-white font-headline font-bold text-sm rounded-xl hover:bg-primary-dark hover:scale-105 active:scale-95 transition-all shadow-md shadow-orange-200 dark:shadow-orange-900/30 whitespace-nowrap">
                Explore All Markets →
              </Link>
            </div>
          </FadeIn>
      ...
      <section className="py-24 px-6 md:px-12 bg-white dark:bg-slate-900">
        <div className="max-w-3xl mx-auto text-center">
          <FadeIn>
            <h2 className="text-4xl md:text-5xl font-headline font-black text-on-surface dark:text-white mb-4">
              Only verified,{' '}
              <span className="text-primary">high-value businesses</span>
            </h2>
            <p className="text-on-surface-variant dark:text-slate-400 mb-14 leading-relaxed max-w-xl mx-auto">
          ...
          <div className="grid grid-cols-3 gap-8">
            {[
              ...
              <FadeIn key={stat.label} delay={i * 150}>
                ...
                <div className="text-xs font-bold uppercase tracking-widest text-on-surface-variant dark:text-slate-500">{stat.label}</div>
              </FadeIn>
            ))}
          </div>
        </div>
      ...
      <section className="py-20 px-6 md:px-12 bg-slate-50 dark:bg-slate-900/50">
        <div className="max-w-5xl mx-auto">
          <FadeIn className="text-center mb-12">
            ...
            <h2 className="text-3xl font-headline font-black text-on-surface dark:text-white">Everything you need to win deals</h2>
          </FadeIn>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {[
              ...
              <FadeIn key={i} delay={i * 80}>
                <div className="feature-card p-6 bg-white dark:bg-slate-800 rounded-2xl border border-outline-variant dark:border-slate-700 h-full">
                  <div className="w-12 h-12 rounded-xl bg-orange-50 dark:bg-orange-950/50 border border-orange-100 dark:border-orange-800 flex items-center justify-center mb-4 group-hover:scale-110">
                    <span className="material-symbols-outlined notranslate text-2xl text-primary" translate="no">{f.icon}</span>
                  </div>
                  <h3 className="font-headline font-bold text-on-surface dark:text-white mb-2">{f.title}</h3>
                  <p className="text-on-surface-variant dark:text-slate-400 text-sm leading-relaxed">{f.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      {/* ══ ENTERPRISE TIERS ══ */}
      <section className="py-20 px-6 md:px-12 bg-white dark:bg-slate-900 transition-colors duration-300">
        <div className="max-w-5xl mx-auto">
          ...
          <FadeIn className="flex justify-center mb-10">
            <div className="flex items-center gap-1 p-1 rounded-xl border border-outline-variant dark:border-slate-700 bg-slate-50 dark:bg-slate-800 transition-colors">
              <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            {/* FREE */}
            <FadeIn delay={0}>
              <div className="tier-card p-7 bg-white dark:bg-slate-800 rounded-2xl border border-outline-variant dark:border-slate-700 h-full flex flex-col transition-colors"
                ...
                <ul className="space-y-2.5 text-sm text-on-surface-variant dark:text-slate-300 mb-8 flex-1 transition-colors">
                  {['List 3 Products — Free Forever', 'Verified Business Profile', 'Receive Real Buyer Inquiries', 'WhatsApp & Email Trade Alerts', 'Taste of Karm AI', 'CRM Access at ₹99 / $1.11'].map((f) => (
                    <li key={f} className="flex items-start gap-2">
                      <span className="w-4 h-4 rounded-full bg-orange-50 dark:bg-orange-950/50 border border-orange-200 dark:border-orange-800 flex items-center justify-center flex-shrink-0 mt-0.5 transition-colors">
            ...
            <FadeIn delay={80}>
              <div className="tier-card p-7 bg-white dark:bg-slate-800 rounded-2xl border border-outline-variant dark:border-slate-700 h-full flex flex-col transition-colors"
                ...
                <p className="text-xs text-green-600 dark:text-green-400 font-bold mb-4">{currency === 'INR' ? 'Yearly ₹9,990 — Save ₹1,998' : 'Yearly $119 — Save $25'}</p>
                <div className="w-8 h-1 rounded-full bg-orange-200 dark:bg-orange-900/50 mb-5 transition-colors" />
                <ul className="space-y-2.5 text-sm text-on-surface-variant dark:text-slate-300 mb-8 flex-1 transition-colors">
            ...
            <FadeIn delay={160}>
              <div className="tier-card p-7 bg-white dark:bg-slate-800 rounded-2xl border border-outline-variant dark:border-slate-700 h-full flex flex-col transition-colors"
                ...
                <p className="text-xs text-green-600 dark:text-green-400 font-bold mb-4">{currency === 'INR' ? 'Yearly ₹39,999 — Save ₹7,989' : 'Yearly $444 — Save $144'}</p>
                <div className="w-8 h-1 rounded-full bg-orange-200 dark:bg-orange-900/50 mb-5 transition-colors" />
                <ul className="space-y-2.5 text-sm text-on-surface-variant dark:text-slate-300 mb-8 flex-1 transition-colors">
            ...
            <FadeIn delay={0}>
              <div className="tier-card relative p-7 rounded-2xl h-full flex flex-col"
                ...
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-slate-900 dark:bg-slate-800 text-white text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full whitespace-nowrap flex items-center gap-1">
                  <span className="material-symbols-outlined notranslate text-amber-300" translate="no" style={{ fontSize: '12px' }}>star</span> Most Popular
                </div>
                <p className="text-white/80 text-xs font-bold uppercase tracking-widest mb-1">Smart Growth</p>
            ...
            <FadeIn delay={80}>
              <div className="tier-card relative p-7 bg-white dark:bg-slate-800 rounded-2xl border-2 border-primary h-full flex flex-col transition-colors"
                ...
                <p className="text-xs text-green-600 dark:text-green-400 font-bold mb-4">{currency === 'INR' ? 'Yearly ₹79,999 — Save ₹1,59,989 (60%+ off)' : 'Yearly $999 — Save $2,001 (60%+ off)'}</p>
                <div className="w-8 h-1 rounded-full bg-orange-200 dark:bg-orange-900/50 mb-5 transition-colors" />
                <ul className="space-y-2.5 text-sm text-on-surface-variant dark:text-slate-300 mb-8 flex-1 transition-colors">
                ...
                <button className="w-full py-2.5 bg-primary text-white font-headline font-bold rounded-xl hover:bg-primary-dark transition-all shadow-md shadow-orange-200 dark:shadow-orange-900/30 text-sm">
                  Go Pro →
                </button>
              </div>
            </FadeIn>
            ...
            <FadeIn delay={160}>
              <div className="tier-card p-7 bg-white dark:bg-slate-800 rounded-2xl border border-outline-variant dark:border-slate-700 h-full flex flex-col transition-colors"
                ...
                <p className="text-xs text-green-600 dark:text-green-400 font-bold mb-4">{currency === 'INR' ? 'Yearly ₹2,97,500 — Save ₹1,34,488' : 'Yearly $4,444 — Save $888'}</p>
                <div className="w-8 h-1 rounded-full bg-orange-200 dark:bg-orange-900/50 mb-5 transition-colors" />
                <ul className="space-y-2.5 text-sm text-on-surface-variant dark:text-slate-300 mb-8 flex-1 transition-colors">
